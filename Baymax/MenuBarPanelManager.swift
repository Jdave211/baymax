...
extension Notification.Name {
    static let baymaxDismissPanel = Notification.Name("baymaxDismissPanel")
    static let baymaxShowPanel = Notification.Name("baymaxShowPanel")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    private func createStatusItem() {
        ...
        button.image = makeBaymaxMenuBarIcon()
        button.image?.isTemplate = true
        button.action = #selector(statusItemClicked)
        button.target = self
    }

    /// Draws the baymax triangle as a menu bar icon. Uses the same shape
    /// and rotation as the in-app cursor so the menu bar icon matches.
    private func makeBaymaxMenuBarIcon() -> NSImage {
