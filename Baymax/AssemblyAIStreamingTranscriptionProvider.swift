...
final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    /// The provider is always configured because it fetches temp tokens from
    /// the worker proxy — no local API key needed.
    ...
    /// Fetches a short-lived AssemblyAI token from the worker proxy.
    private func fetchTemporaryToken() async throws -> String {
        ...
        guard let tokenURL = URL(string: "\(workerBaseURL)/transcribe-token") else {
            throw AssemblyAIStreamingTranscriptionProviderError(message: "Invalid worker URL for transcribe-token")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw AssemblyAIStreamingTranscriptionProviderError(
                message: "Failed to fetch AssemblyAI token: \(errorBody)"
            )
        }
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw AssemblyAIStreamingTranscriptionProviderError(
                message: "Invalid token response from worker"
            )
        }
    ...
    func startStreamingSession(
        ...
        BaymaxDebugLog.log("AssemblyAI: Fetched temporary token (length: \(temporaryToken.count))")
...
private final class AssemblyAIStreamingTranscriptionSession: NSObject, BuddyStreamingTranscriptionSession {
    ...
    func open() async throws {
        ...
        print("🔌 AssemblyAI: Opening websocket to \(websocketURL.host ?? "unknown")...")
        BaymaxDebugLog.log("AssemblyAI: Opening websocket to \(websocketURL.host ?? "unknown")")

        var websocketRequest = URLRequest(url: websocketURL)
        if let apiKey {
            websocketRequest.setValue(apiKey, forHTTPHeaderField: "Authorization")
            print("🔌 AssemblyAI: Using API key (length: \(apiKey.count))")
            BaymaxDebugLog.log("AssemblyAI: Using API key (length: \(apiKey.count))")
        } else {
            print("⚠️ AssemblyAI: No API key set — websocket auth will likely fail")
            BaymaxDebugLog.log("AssemblyAI: NO API KEY — will fail")
        }
    ...
    private func handleIncomingTextMessage(_ text: String) {
        ...
        do {
            ...
            case "begin":
                print("🔌 AssemblyAI: Received 'begin' — websocket connected and ready")
                BaymaxDebugLog.log("AssemblyAI: websocket 'begin' received — ready")
            ...
            case "termination":
                print("🔌 AssemblyAI: Received 'termination'")
                BaymaxDebugLog.log("AssemblyAI: websocket 'termination' received")
                resolveReadyContinuationIfNeeded(with: .success(()))
                stateQueue.async {
            ...
            case "error":
                ...
                print("❌ AssemblyAI: Server error — \(messageText)")
                BaymaxDebugLog.log("AssemblyAI ERROR: \(messageText)")
    ...
    private func failSession(with error: Error) {
        BaymaxDebugLog.log("AssemblyAI: failSession — \(error.localizedDescription)")
