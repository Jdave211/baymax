...
class ClaudeAPI {
    ...
    init(apiKey: String, model: String = "claude-sonnet-4-6") {
        ...
        self.workerChatURL = URL(string: "\(workerBaseURL)/chat")!
        self.model = model
    ...
    private func makeAPIRequest() -> URLRequest {
        var request = URLRequest(url: workerChatURL)
    ...
    private func warmUpTLSConnectionIfNeeded() {
        ...
        guard var warmupURLComponents = URLComponents(url: workerChatURL, resolvingAgainstBaseURL: false) else {
            return
        }
    ...
    func analyzeImageStreaming(
        ...
        // Use a structured system block with cache_control so Anthropic caches
        // the (large, static) system prompt and charges only 10% of the input price
        // on subsequent calls within 5 minutes. Saves ~53% on multi-turn conversations.
    ...
    /// Lightweight text-only call for background tasks like summarization.
    /// Uses Haiku (cheapest model) regardless of the instance's selected model,
    /// keeping summarization cost under $0.0001 per call.
    func summarizeText(prompt: String) async throws -> String {
        var request = makeAPIRequest()
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(
                ...
                userInfo: [NSLocalizedDescriptionKey: "Summarization error: \(responseString)"]
            )
        }
        ...
        guard let content = json?["content"] as? [[String: Any]],
            ...
            throw NSError(
                domain: "ClaudeAPI",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: "Invalid summarization response format"]
            )
        }
