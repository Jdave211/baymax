...
enum BuddyDictationError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noMicrophoneAvailable:
            return "no microphone detected. connect a mic or headset and try again."
        }
    }
}
...
final class BuddyDictationManager: NSObject, ObservableObject {
    ...
    /// If no speech is detected within this many seconds of starting a recording,
    /// the session is cancelled and the user is told no audio was detected. This
    /// handles the case where there's no microphone connected (e.g. Mac Mini).
    private static let noSpeechDetectedTimeoutSeconds: TimeInterval = 8.0
    ...
    private var noSpeechDetectedTimeoutWorkItem: DispatchWorkItem?
    /// Tracks whether any non-silent audio has been received during the current
    /// recording session. Used by the no-speech timeout to distinguish "no mic"
    /// from "user hasn't spoken yet."
    private var hasReceivedNonSilentAudioDuringSession = false
    ...
    private func startRecognitionSession() async throws {
        ...
        // Check if any audio input device is available before trying to record.
        // On Macs without a built-in mic (e.g. Mac Mini with no external mic),
        // the audio engine may start but only capture silence, causing the app
        // to hang in "listening" state forever.
        let availableAudioInputDevices = AVCaptureDevice.DiscoverySession(
            deviceTypes: [.microphone, .external],
            mediaType: .audio,
            position: .unspecified
        ).devices
        if availableAudioInputDevices.isEmpty {
            throw BuddyDictationError.noMicrophoneAvailable
        }
        ...
        let activeTranscriptionSession = try await transcriptionProvider.startStreamingSession(
            ...
            onTranscriptUpdate: { [weak self] transcriptText in
                Task { @MainActor in
                    ...
                    self?.hasReceivedNonSilentAudioDuringSession = true
                    // Cancel the no-speech timeout once we get any transcript
                    self?.noSpeechDetectedTimeoutWorkItem?.cancel()
                    self?.noSpeechDetectedTimeoutWorkItem = nil
                }
            },
        ...
        hasReceivedNonSilentAudioDuringSession = false
        scheduleNoSpeechDetectedTimeout()
    }

    /// Schedules a timeout that fires if no transcription updates arrive within
    /// the timeout window. Handles the case where the audio engine is running
    /// but no usable audio is being captured (no mic, mic muted, etc.).
    private func scheduleNoSpeechDetectedTimeout() {
        noSpeechDetectedTimeoutWorkItem?.cancel()
        let timeoutWorkItem = DispatchWorkItem { [weak self] in
            Task { @MainActor in
                guard let self else { return }
                guard self.isDictationInProgress else { return }
                guard !self.hasReceivedNonSilentAudioDuringSession else { return }

                print("⚠️ BuddyDictationManager: no speech detected within timeout — cancelling session")
                BaymaxDebugLog.log("DictationManager: no-speech timeout fired — cancelling")
                self.lastErrorMessage = "no audio detected. check that your microphone is connected and working."
                self.cancelCurrentDictation(preserveDraftText: false)
            }
        }
        noSpeechDetectedTimeoutWorkItem = timeoutWorkItem
        DispatchQueue.main.asyncAfter(
            deadline: .now() + Self.noSpeechDetectedTimeoutSeconds,
            execute: timeoutWorkItem
        )
    }

    private func handleRecognitionError(_ error: Error) {
        BaymaxDebugLog.log("DictationManager: recognition error — \(error.localizedDescription)")
    ...
    private func finishCurrentDictationSessionIfNeeded(shouldSubmitFinalDraft: Bool) {
        guard !hasFinishedCurrentDictationSession else {
            print("🎙️ BuddyDictationManager: finishCurrentDictation skipped — already finished")
            return
        }
        ...
        print("🎙️ BuddyDictationManager: finishing session — transcript='\(finalTranscriptText)', shouldSubmit=\(shouldSubmitFinalDraft)")
        BaymaxDebugLog.log("DictationManager: finishing — transcript='\(finalTranscriptText)', shouldSubmit=\(shouldSubmitFinalDraft)")
        ...
        guard shouldSubmitFinalDraft else {
            print("🎙️ BuddyDictationManager: not submitting — shouldSubmitFinalDraft is false")
            return
        }
        guard !finalTranscriptText.isEmpty else {
            print("🎙️ BuddyDictationManager: not submitting — transcript is empty")
            return
        }

        print("🎙️ BuddyDictationManager: submitting final transcript to Claude pipeline")
    ...
    private func resetSessionState() {
        ...
        finalizeFallbackWorkItem = nil
        noSpeechDetectedTimeoutWorkItem?.cancel()
        noSpeechDetectedTimeoutWorkItem = nil
        hasReceivedNonSilentAudioDuringSession = false
        activeTranscriptionSession = nil
        draftCallbacks = nil
