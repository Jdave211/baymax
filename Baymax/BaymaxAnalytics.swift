...
enum BaymaxAnalytics {

    // MARK: - Setup

    static func configure() {
        guard let apiKey = DotEnv.get("POSTHOG_API_KEY") ?? ProcessInfo.processInfo.environment["POSTHOG_API_KEY"],
              !apiKey.isEmpty else {
            return
        }
        let host = DotEnv.get("POSTHOG_HOST") ?? ProcessInfo.processInfo.environment["POSTHOG_HOST"] ?? "https://us.i.posthog.com"
    ...
    /// The 40s onboarding demo interaction where Baymax points at something.
